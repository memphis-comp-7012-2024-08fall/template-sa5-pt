...
RSpec.describe 'tracks/edit.html.erb' do
  ...
  #   expect(Nokogiri::HTML.parse(rendered).search('*').map(&:name)).to eq %w[html body h1 form input div
  #                                                                           label input div label input input p a]
  #   expect(Nokogiri::HTML.parse(rendered).text.gsub(/\s+/, '')).to eq 'EditTrackformyalbumTitleLengthinsecondsBack'
  # end
end
