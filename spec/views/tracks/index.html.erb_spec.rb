...
RSpec.describe 'tracks/index.html.erb' do
  ...
  #   pp Nokogiri::HTML.parse(rendered).search('*').map(&:name)
  #   expect(Nokogiri::HTML.parse(rendered).search('*').map(&:name)).to eq %w[html body h1 div table thead tr th th th
  #                                                                           tbody tr td td td a a form input button tr
  #                                                                           td td td a a form input button p a p a]
  # end
end
